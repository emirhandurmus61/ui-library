...
export interface FooterMultiColumnProps {
  ...
  /** Alt bant arka plan rengi — varsayılan surface */
  bottomVariant?: "surface" | "muted";
  className?: string;
}
  ...
  return (
    <footer className={cn("w-full border-t border-border bg-surface", className)}>
                ...
                {col.links.map((link) => (
                  <li key={link.label}>
                    ...
                    >
                      {link.label}
                      {link.badge && (
                        <span className="inline-flex items-center px-1.5 py-0.5 rounded-full text-[10px] font-semibold bg-primary-subtle text-primary">
                          {link.badge}
                        </span>
                      )}
                    </a>
      ...
      <div className={cn(
        ...
        bottomVariant === "muted" ? "bg-background-muted" : "bg-surface"
      )}>
