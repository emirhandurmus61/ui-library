"use client";

import { useState } from "react";
import { cn } from "@/lib/utils";
import { Input } from "@/components/ui/input";
import { PasswordInput } from "@/components/ui/input";
import { Button } from "@/components/ui/button";
import { Checkbox } from "@/components/ui/checkbox";

function MailIcon() {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" width="1em" height="1em">
      <rect width="20" height="16" x="2" y="4" rx="2"/><path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7"/>
    </svg>
  );
}

function UserIcon() {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" width="1em" height="1em">
      <circle cx="12" cy="8" r="4"/><path d="M20 21a8 8 0 1 0-16 0"/>
    </svg>
  );
}

function GoogleIcon() {
  return (
    <svg viewBox="0 0 24 24" width="1em" height="1em">
      <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/>
      <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/>
      <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/>
      <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/>
    </svg>
  );
}

function GithubIcon() {
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" width="1em" height="1em">
      <path d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0 1 12 6.844a9.59 9.59 0 0 1 2.504.337c1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0 0 22 12.017C22 6.484 17.522 2 12 2z"/>
    </svg>
  );
}

function getStrength(pw: string): { score: number; label: string; color: string; textColor: string } {
  if (!pw) return { score: 0, label: "", color: "", textColor: "" };
  let s = 0;
  if (pw.length >= 8)           s++;
  if (pw.length >= 12)          s++;
  if (/[A-Z]/.test(pw))         s++;
  if (/[0-9]/.test(pw))         s++;
  if (/[^A-Za-z0-9]/.test(pw))  s++;
  if (s <= 1) return { score: s, label: "Çok zayıf",  color: "bg-danger",  textColor: "text-danger" };
  if (s === 2) return { score: s, label: "Zayıf",      color: "bg-warning", textColor: "text-warning" };
  if (s === 3) return { score: s, label: "Orta",       color: "bg-warning", textColor: "text-warning" };
  if (s === 4) return { score: s, label: "Güçlü",      color: "bg-success", textColor: "text-success" };
  return              { score: s, label: "Çok güçlü",  color: "bg-success", textColor: "text-success" };
}

export interface RegisterCardProps {
  logoText?: string;
  logo?: React.ReactNode;
  title?: string;
  subtitle?: string;
  onSubmit?: (data: { name: string; email: string; password: string; terms: boolean }) => void | Promise<void>;
  onGoogleSignup?: () => void;
  onGithubSignup?: () => void;
  loginHref?: string;
  termsHref?: string;
  privacyHref?: string;
  showSocials?: boolean;
  showPasswordStrength?: boolean;
  className?: string;
}

export function RegisterCard({
  logoText = "Brand",
  logo,
  title = "Hesap oluşturun",
  subtitle = "Ücretsiz başlayın, kredi kartı gerekmez",
  onSubmit,
  onGoogleSignup,
  onGithubSignup,
  loginHref = "#",
  termsHref = "#",
  privacyHref = "#",
  showSocials = true,
  showPasswordStrength = true,
  className,
}: RegisterCardProps) {
  const [name, setName]         = useState("");
  const [email, setEmail]       = useState("");
  const [password, setPassword] = useState("");
  const [terms, setTerms]       = useState(false);
  const [loading, setLoading]   = useState(false);
  const [errors, setErrors]     = useState<Record<string, string>>({});

  const clear = (field: string) => setErrors(p => ({ ...p, [field]: "" }));

  const validate = () => {
    const e: Record<string, string> = {};
    if (!name.trim()) e.name = "Ad Soyad zorunludur.";
    if (!email.trim()) e.email = "E-posta zorunludur.";
    else if (!/\S+@\S+\.\S+/.test(email)) e.email = "Geçerli bir e-posta girin.";
    if (!password) e.password = "Şifre zorunludur.";
    else if (password.length < 8) e.password = "En az 8 karakter olmalıdır.";
    if (!terms) e.terms = "Devam etmek için şartları kabul edin.";
    return e;
  };

  const handleSubmit = async (ev: React.FormEvent) => {
    ev.preventDefault();
    const errs = validate();
    if (Object.keys(errs).length) { setErrors(errs); return; }
    setErrors({});
    setLoading(true);
    try { await onSubmit?.({ name, email, password, terms }); }
    finally { setLoading(false); }
  };

  const { score, label, color, textColor } = getStrength(password);

  return (
    <div className={cn(
      "w-full max-w-sm flex flex-col gap-6",
      "bg-surface border border-border rounded-[var(--radius-xl)] shadow-lg p-8",
      className
    )}>
      {/* Logo + başlık */}
      <div className="flex flex-col items-center gap-3 text-center">
        {logo ?? (
          <span className="flex size-10 items-center justify-center rounded-[var(--radius-lg)] bg-primary text-primary-foreground text-base font-bold">
            {logoText[0]}
          </span>
        )}
        <div>
          <h1 className="text-xl font-bold text-foreground">{title}</h1>
          <p className="text-sm text-foreground-muted mt-0.5">{subtitle}</p>
        </div>
      </div>

      {/* Sosyal kayıt */}
      {showSocials && (
        <>
          <div className="flex flex-col gap-2">
            <Button type="button" variant="outline" className="w-full" leftIcon={<GoogleIcon />} onClick={onGoogleSignup}>
              Google ile kayıt ol
            </Button>
            <Button type="button" variant="outline" className="w-full" leftIcon={<GithubIcon />} onClick={onGithubSignup}>
              GitHub ile kayıt ol
            </Button>
          </div>
          <div className="flex items-center gap-3">
            <span className="flex-1 border-t border-border" />
            <span className="text-xs text-foreground-subtle">ya da e-posta ile</span>
            <span className="flex-1 border-t border-border" />
          </div>
        </>
      )}

      {/* Form */}
      <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-4">
        <Input
          label="Ad Soyad"
          placeholder="Emirhan Durmuş"
          value={name}
          onChange={(e) => { setName(e.target.value); clear("name"); }}
          leftIcon={<UserIcon />}
          errorText={errors.name}
          required
          autoComplete="name"
        />

        <Input
          label="E-posta"
          type="email"
          placeholder="ornek@email.com"
          value={email}
          onChange={(e) => { setEmail(e.target.value); clear("email"); }}
          leftIcon={<MailIcon />}
          errorText={errors.email}
          required
          autoComplete="email"
        />

        <div className="flex flex-col gap-1.5">
          <PasswordInput
            label="Şifre"
            placeholder="En az 8 karakter"
            value={password}
            onChange={(e) => { setPassword(e.target.value); clear("password"); }}
            errorText={errors.password}
            required
            autoComplete="new-password"
          />
          {showPasswordStrength && password && (
            <div className="flex flex-col gap-1.5">
              <div className="flex gap-1">
                {[1, 2, 3, 4].map((i) => (
                  <div key={i} className={cn("h-1 flex-1 rounded-full transition-all duration-300", i <= Math.min(score, 4) ? color : "bg-border")} />
                ))}
              </div>
              <p className={cn("text-xs", textColor)}>{label}</p>
            </div>
          )}
        </div>

        <Checkbox
          label={
            <span>
              <a href={termsHref} className="text-primary hover:underline">Kullanım Şartları</a>
              {" "}ve{" "}
              <a href={privacyHref} className="text-primary hover:underline">Gizlilik Politikası</a>
              {"'nı okudum ve kabul ediyorum."}
            </span> as unknown as string
          }
          checked={terms}
          onChange={(e) => { setTerms(e.target.checked); clear("terms"); }}
          errorText={errors.terms}
        />

        <Button type="submit" className="w-full" loading={loading}>
          Hesap Oluştur
        </Button>
      </form>

      <p className="text-center text-sm text-foreground-muted">
        Zaten hesabınız var mı?{" "}
        <a href={loginHref} className="text-primary font-medium hover:underline">
          Giriş yapın
        </a>
      </p>
    </div>
  );
}
